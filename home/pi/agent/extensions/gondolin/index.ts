...
function configuredHttpHosts(): string[] {
  const configured = process.env.PI_GONDOLIN_HTTP_HOSTS?.trim();
  if (!configured) return DEFAULT_HTTP_HOSTS;
...
function vmMemory(): string {
  const configured = process.env.PI_GONDOLIN_MEMORY?.trim();
  return configured || DEFAULT_VM_MEMORY;
}

function vmCpus(): number {
  const configured = Number.parseInt(process.env.PI_GONDOLIN_CPUS ?? "", 10);
...
function configuredGuestImagePath(): string | undefined {
  ...
  if (!cacheHome) return undefined;

  const defaultGuestDir = path.join(cacheHome, "pi-gondolin", "guest");
  return existsSync(path.join(defaultGuestDir, "manifest.json"))
    ? defaultGuestDir
    : undefined;
}

function repositoryFromSshUrl(value: string): string | null {
  const scpStyle = value.match(/^git@github\.com:([A-Za-z0-9._/-]+)$/i);
  if (scpStyle) return scpStyle[1].replace(/^\/+|\/+$/g, "");
...
function protectsPiControlPlane(
  op: string,
  providerPath: string,
  flags?: string,
): boolean {
  const inProjectPiDirectory =
    providerPath === "/.pi" || providerPath.startsWith("/.pi/");
  if (!inProjectPiDirectory) return false;
  if (op === "open") return isWriteFlag(flags ?? "r");
  return ["mkdir", "rmdir", "unlink", "rename", "symlink"].includes(op);
}

function configuredGitRepositories(localCwd: string): Set<string> {
  const configured = (process.env.PI_GONDOLIN_GIT_REPOSITORIES ?? "")
    .split(",")
    .map((value) => value.trim().replace(/^\/+|\/+$/g, ""))
    .filter(Boolean);

  try {
    const gitConfig = readFileSync(path.join(localCwd, ".git", "config"), "utf8");
    let inOrigin = false;
    for (const line of gitConfig.split("\n")) {
      ...
      const originUrl = line.match(/^\s*url\s*=\s*(.+?)\s*$/i)?.[1];
      const repository = originUrl && repositoryFromSshUrl(originUrl);
      if (repository) configured.push(repository);
    }
  } catch {
    // A non-Git workspace has no implicit SSH repository access.
  }
...
export default function (pi: ExtensionAPI) {
  ...
  async function startVm(ctx?: ExtensionContext): Promise<VM> {
    ...
    const { httpHooks, env } = createHttpHooks({
      // Nix flakes need outbound HTTPS for registries, source fetches, and
    ...
    const created = await VM.create({
      ...
      env: {
        ...env,
        // Make flake-enabled Nix commands work even if the guest image has not
        // yet written /etc/nix/nix.conf.
        NIX_CONFIG,
        // The guest connects to Gondolin's SSH terminator, whose ephemeral host
        // key is distinct from GitHub's separately verified upstream key.
      ...
      ssh: {
        allowedHosts: ALLOWED_SSH_HOSTS,
        // Gondolin terminates SSH on the host. The socket and key are never
        // mounted into the guest.
        agent: process.env.SSH_AUTH_SOCK,
        execPolicy: async (request) => {
          const git = getInfoFromSshExecRequest(request);
          if (!git) {
            return {
              allow: false,
              message: "Only Git smart-protocol SSH commands are allowed",
            };
          }
          if (!gitRepositories.has(git.repo)) {
            return {
              allow: false,
              message: `SSH access to ${git.repo} is not allowed`,
            };
          }

          if (
            git.service === "git-upload-pack" ||
            git.service === "git-receive-pack"
          ) {
            return { allow: true };
          }
          ...
          return {
            allow: false,
            message: `Git SSH service ${git.service} is not allowed`,
          };
        },
      },
      vfs: {
        mounts: {
          // Project files are writable, but project-local Pi configuration is
          // host control-plane input: extensions placed there execute on the
          // host after reload and must not be mutable from the guest.
          [GUEST_WORKSPACE]: new ShadowProvider(
            new RealFSProvider(localCwd),
            {
              shouldShadow: ({ op, path: providerPath, flags }) =>
                protectsPiControlPlane(op, providerPath, flags),
              writeMode: "deny",
            },
          ),
        },
    ...
    await created.exec([
      ...
      `if command -v nix >/dev/null 2>&1; then
        mkdir -p /etc/nix
        printf '%s\n' ${JSON.stringify(NIX_CONFIG)} > /etc/nix/nix.conf
      fi
    ...
    handler: async (_args, ctx) => {
      ...
      ctx.ui.notify(
        [
          ...
          `Resources: ${vmMemory()}, ${vmCpus()} CPUs`,
          "HTTP(S): blocked",
          `SSH: ${ALLOWED_SSH_HOSTS.join(", ")} (Git transport only)`,
          `Git repositories: ${[...configuredGitRepositories(localCwd)].join(", ") || "none"}`,
          `Shell: ${shellPath}`,
        ].join("\n"),
        "info",
      );
